...
use crate::bindings::windows::win32::{
    gdi::HBRUSH,
    menus_and_resources::{HCURSOR, HICON, HMENU},
    system_services::{HINSTANCE, LRESULT, PWSTR, WINDOWSCLASS_STYLES},
    windows_and_messaging::{
        CreateWindowExW, DefWindowProcW, RegisterClassExW, CREATESTRUCTW, HWND, LPARAM,
        WINDOWS_EX_STYLE, WINDOWS_STYLE, WM_CREATE, WM_NCDESTROY, WNDCLASSEXW, WPARAM,
    },
};

use crate::win32impl::{GetWindowLongPtrW, SetWindowLongPtrW, GWLP_USERDATA};
...
impl<'a> WindowBuilder<'a> {
    ...
    pub fn new(
        ...
        // TODO: Remove once `CW_USEDEFAULT` is generated from the metadata again
        const CW_USEDEFAULT: i32 = 0x8000_0000_u32 as i32;
        WindowBuilder {
            window_proc: Rc::new(Box::new(window_proc)),
            dwExStyle: WINDOWS_EX_STYLE::default(),
            window_class,
            window_name: Vec::new(),
            dwStyle: WINDOWS_STYLE::default(),
            x: CW_USEDEFAULT,
            y: CW_USEDEFAULT,
    ...
    pub fn build(self) -> HWND {
        unsafe {
            let wnd_proc_ptr = Rc::into_raw(self.window_proc) as *mut _;
    ...
    pub fn style(mut self, style: WINDOWS_STYLE) -> Self {
        self.dwStyle = style;
        self
    }
    ...
    pub fn ex_style(mut self, style: WINDOWS_EX_STYLE) -> Self {
...
extern "system" fn raw_window_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
    ...
    unsafe {
        if msg == WM_CREATE {
            ...
            let window_state_ptr = create_struct.lp_create_params;
...
impl WindowClass {
    ...
    pub fn builder(class_name: impl AsRef<OsStr>) -> WindowClassBuilder {
        WindowClassBuilder {
            ...
            style: WINDOWSCLASS_STYLES::default(),
            cbWndExtra: 0,
            hInstance: HINSTANCE::default(),
            hIcon: HICON::default(),
    ...
    fn as_lpcwstr(&self) -> PWSTR {
        match self {
            WindowClass::Atom(atom) => PWSTR(*atom as *mut _),
...
impl WindowClassBuilder {
    ...
    pub fn build(self) -> Result<WindowClass, Error> {
        unsafe {
            let wnd = WNDCLASSEXW {
                ...
                style: self.style.0,
                lpfn_wnd_proc: Some(raw_window_proc),
                cb_cls_extra: 0,
                cb_wnd_extra: 0,
                h_instance: self.hInstance,
                h_icon: self.hIcon,
                h_cursor: self.hCursor,
                hbr_background: self.hbrBackground,
                lpsz_menu_name: PWSTR(pointer_or_null(&self.menu_name) as *mut _),
                lpsz_class_name: PWSTR(self.class_name.as_ptr() as *mut _),
                h_icon_sm: self.hIconSm,
            };
    ...
    pub fn class_style(mut self, style: WINDOWSCLASS_STYLES) -> Self {
        self.style = style;
        self
    }
